...
const library: LibraryItem[] = [
  {
    ...
    tags: ["需求", "诊断", "沟通", "课程"],
    courseIds: ["growth-day-1", "aesthetic-day-1"],
  },
  {
    id: "value",
    kind: "工具",
  ...
  return (
    <main className="site-shell">
      <header className="topbar">
        <button className="brand" onClick={() => setView("dashboard")} aria-label="返回今日学习">
          <span className="brand-name">蓝筱玉整装学堂</span>
          <span className="brand-subtitle">学习成长中心</span>
        </button>

        <DesktopNav currentView={view} onNavigate={setView} />
    ...
    items={library.filter((item) => item.kind === "工具")}
      ...
      {view === "assignments" && (
        <div className="page content-page">
          <PageIntro eyebrow="学习闭环 · 作业与通关" title="把练习提交出去，才会得到下一步反馈" description="作业按未开始、进行中、已提交、待批改、需修改、已通过管理；第一阶段先建立清晰入口，提交与批改接口随后接入。" />
          <section className="assignment-board">
            {[
              { title: "DAY 1｜客户需求诊断", course: "7天业绩倍增突击营", status: "待提交", detail: "完成三层提问，写出客户真正关心的问题与下一步动作。" },
              { title: "DAY 2｜美学判断练习", course: "全案色彩美学精华班", status: "进行中", detail: "用美学评分表完成一张真实空间图的客观拆解。" },
              { title: "课后反馈确认", course: "当前学习任务", status: "待批改", detail: "提交文字、图片或视频后，在这里查看助教反馈与修改要求。" },
            ].map((assignment) => (
              <article className="assignment-card" key={assignment.title}>
                <div><small>{assignment.course}</small><h2>{assignment.title}</h2><p>{assignment.detail}</p></div>
                <span className="assignment-status">{assignment.status}</span>
                <button type="button" onClick={() => notify("第一阶段已建立作业入口，提交与批改接口将在下一阶段接入。")}>查看作业要求 →</button>
              </article>
            ))}
          </section>
        </div>
      ...
      {view === "profile" && (
        <div className="page content-page">
          <PageIntro eyebrow="学员账户" title="我的学习档案" description="查看当前身份、课程权限与学习闭环状态。" />
          <section className="profile-summary">
            <div><small>学员姓名</small><strong>{authUser?.user_metadata?.display_name || "尚未登录"}</strong></div>
            <div><small>登录账号</small><strong>{authUser?.email || "请先登录"}</strong></div>
            <div><small>已开通课程</small><strong>{courseAccessIds.length} 门</strong></div>
            {!authUser && <button type="button" onClick={() => openAuth("login")}>登录学习中心 →</button>}
          </section>
        </div>
      )}
